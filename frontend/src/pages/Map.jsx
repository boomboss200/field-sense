...
const Map = () => {
  ...
  const closePopupOnOutsideClick = (event) => {
    if (isImportPopupOpen && popupRef.current && !popupRef.current.contains(event.target)) {
  ...
  const postKml = async () => {
    ...
    try {
      const response = await axios.post(`http://127.0.0.1:8000/fields/addfield/${farmid}/`, data, {
        headers: {
      ...
      //   // Now 'cropFieldStatus' holds the desired status
        ...
        toast.info("Fields Successfully added to the farm. ", {
...
const makePredictions = async () => {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      const responseData = response.data['Model_Predictions'];
...
<div className="flex items-center justify-center space-x-2">
  <img
    ...
    className="h-20 w-20"
  />
  <span className="font-extrabold text-2xl text-green-700 px-6 py-1">Add Fields</span>
</div>
</div>
...
<div class="flex w-full py-6">
  ...
  <button class="flex-shrink-0 bg-green-600 hover:bg-green-800 text-gray-100 px-3 py-2 rounded-lg shadow-lg text-sm mr-4"
    type='button'
...
<div class="flex w-full justify-center py-6">
                      <button class="w-1/2 bg-green-600 hover:bg-green-800  text-gray-100 px-3 py-2 rounded-lg shadow-lg text-sm"
                        id="submitform"
                        type='submit'
  ...
  <text className='font-lighter text-sm items-right text-gray-500  px-2'>Satisfied with the field? Click "Add Field" to successfully add field to the farm </text>
          {/* ADD FIELD BUTTON */}
                  <div class="flex w-full justify-center py-6">
                      <button class="w-full bg-emerald-600 hover:bg-emerald-900 border-emerald-600 text-gray-100 px-3 py-2 rounded-lg shadow-lg text-sm"
                        id="submitform"
                        type='submit'
                        onClick={createField}
                      >
...
<Popup open={isImportPopupOpen} modal nested>
{
  ...
  <img
    ...
    className="h-20 w-20"
  />
  <span className="font-extrabold text-2xl text-green-700 px-4 py-1">Import KML</span>
</div>
...
<div class="mt-6 flex flex-row gap-4 px-6 justify-center">
  <button onClick={postKml} class="bg-emerald-600 hover:bg-emerald-900 border-emerald-600  border-emerald-600 text-gray-100 px-3 py-2 rounded-lg shadow-lg text-sm flex gap-2 items-center">
    <div>
